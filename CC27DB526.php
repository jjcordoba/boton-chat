<?php
if(!class_exists("CC27DB526")) {
	class CC27DB526 {
    	public $key = "F463E79ED726A20A";
    	private $product_id = "1";
    	private $product_base = "Boton chat";
    	private $server_host = "https://licencias.bacanhost.com/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_Boton whatsapp_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=Boton whatsapp_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
